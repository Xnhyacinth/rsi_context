...
rankers at H0 0.375. Explore offset 72 submitted five invalid compilers
(`TypeError` on a guessed `retrieve_by_query` signature) after source-order
H0 0.500. Dual-role v4 remains the first scored researcher rewrite on homemade
items, also with `discovery_gain=0`.

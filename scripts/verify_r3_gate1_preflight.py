...
DEFAULT_MANIFEST = PROJECT_ROOT / "configs/r3_gate1_offline_preflight_v1.json"
MANIFEST_GIT_PATH = "configs/r3_gate1_offline_preflight_v1.json"
...
def verify_inputs(
    manifest: dict[str, Any], *, repo_root: Path, resource_root: Path
) -> dict[str, object]:
    """Check bytes and planned budget without loading policy code or using APIs."""

    if manifest.get("schema_version") != 1 or manifest.get("status") != "offline_preflight_only":
        raise ValueError("expected the version-1 offline preflight manifest")
    if manifest.get("baseline_commit") != BASELINE_COMMIT:
        raise ValueError("baseline commit differs from Gate-1 v1")
    ...
    if tracked.keys() != TRACKED_INPUTS or visible.keys() != VISIBLE_INPUTS:
        raise ValueError("manifest input file set differs from Gate-1 v1")
    ...
    if pilot != EXPECTED_PILOT:
        raise ValueError("candidate_pilot differs from Gate-1 v1 settings")
...
def main() -> int:
    ...
    try:
        ...
        result = verify_inputs(manifest, repo_root=PROJECT_ROOT, resource_root=args.resource_root)

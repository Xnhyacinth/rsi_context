#!/usr/bin/env python3
"""Guarded R18 short-input KEP feasibility calibration; explicit execution only."""
...
_REGISTRATION_REL = "configs/r18_kep_short_calibration_v1.json"
_LAUNCH_REL = "configs/r18_kep_short_paid_launch_v1.json"
...
_BOUND_FILES = (
    "scripts/r18_kep_calibration_paid_runner.py",
    "scripts/r18_kep_calibration_offline.py",
    ...
    "src/rsicontext/analysis/k8s_calibration_r18.py",
...
def _read_launch() -> tuple[dict[str, object], str]:
    ...
    if raw != _committed_bytes(_LAUNCH_REL):
        raise ValueError("R18 launch differs from committed HEAD")
    ...
    if (
        ...
        or value["scope"] != "r18-kep-short-input-paid-calibration"
    ):
        raise ValueError("R18 launch schema or scope differs")
    ...
    if (
        ...
        raise ValueError("R18 registration differs from committed launch")
    bound = value["bound_file_sha256"]
    if not isinstance(bound, dict) or set(bound) != set(_BOUND_FILES):
        raise ValueError("R18 bound code list differs")
    for relative in _BOUND_FILES:
        current = (ROOT / relative).read_bytes()
        if current != _committed_bytes(relative) or bound[relative] != _sha(current):
            raise ValueError(f"R18 producer differs from committed launch: {relative}")
    if (
        value["r16_task_sha256"] != R16_TASK_SHA256
        or value["r17_task_sha256"] != R17_TASK_SHA256
        or value["registry_sha256"] != REGISTRY_SHA256
        ...
        raise ValueError("R18 launch identities or call caps differ")
    ...
    *,
    ...
    def preflight(prompt: str) -> dict[str, object]:
        ...
        if item is None or item.get("case") != active_case:
            raise ValueError("R18 task worker prompt is unregistered or out of order")
        return {
            "stage": active_case,
    ...
    rule, _spans = projected_rule(source_root)
...
def run_guarded(
    *,
    ...
    result: dict[str, object] = {
        "schema_version": 1,
        "scope": "r18-kep-short-input-paid-calibration",
    ...
    try:
        ...
        if source_check._source_identity(source_root) != {
            ...
            raise ValueError("R18 KEP source identity differs from registry pin")
        observations = {
        ...
        if (
            ...
            raise ValueError("R18 material or prompt registration drifted")
        ...
        if pre.get("status") != "passed":
            ...
            if (
                ...
                raise RuntimeError("R18 worker attempts or usage differ from journal")
            ...
            if observed_task_usage["unknown_usage_attempts"] != 0:
            ...
            elif task["status"] != "completed-calibration":
                ...
                result["status"] = (
                    "completed-synthetic-calibration" if synthetic else "completed-paid-calibration"
                ) if post.get("status") == "passed" else "post-canary-failed"
                if result["status"] in {
                    "completed-synthetic-calibration",
                    "completed-paid-calibration",
                }:
                    if (
                        ...
                        raise RuntimeError("R18 completed block lacks registered attempts")
                    _read_launch()
                    if source_check._source_identity(source_root) != {
                        "revision": SOURCE_REVISION,
                        "checkout": "detached-clean",
                        ...
                        raise RuntimeError("R18 source or observation changed during block")
        ...
        result["provider_block_valid"] = (
            result["status"] == "completed-paid-calibration" and not synthetic
        )
    except Exception as exc:

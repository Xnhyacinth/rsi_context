#!/usr/bin/env python3
"""Run the capped Siflow PG fixed-reader development screen after preflight."""
...
from rsicontext.analysis.postgresql_siflow_pilot import run_development_pilot  # noqa: E402
...
def _source_identity(revision: str, root: Path) -> dict[str, object]:
    ...
    if git("rev-parse", "HEAD") != SOURCE_REVISIONS[revision]:
        raise RuntimeError(f"PostgreSQL {revision}: source revision changed")
    if git("status", "--porcelain"):
        raise RuntimeError(f"PostgreSQL {revision}: source checkout is dirty")
    ...
    if symbolic.returncode != 1:
        raise RuntimeError(f"PostgreSQL {revision}: source checkout must be detached")
    return {"revision": SOURCE_REVISIONS[revision], "selected_file_sha256": SOURCE_SHA256[revision]}
...
def main() -> int:
    ...
    if (
        ...
        or reference.get("scope") != "pg16_17_full_source_fixed_policy_local_geometry_only"
    ):
        raise RuntimeError("geometry reference does not match frozen PG worker profile")
    roots: dict[str, Path] = {}
    ...
    sessions = {
        revision: build_postgresql_source_contrast_sessions(root, revision=revision)
    ...
    result = run_development_pilot(
        ...
        profile=profile,
        endpoint=endpoint,
        preflight=lambda prompt: measure_pg_worker_prompt(tokenizer, profile, prompt),

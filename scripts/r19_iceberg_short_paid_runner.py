#!/usr/bin/env python3
"""Guarded Siflow screen for two short Iceberg S2 rule applications."""
...
from rsicontext.analysis.iceberg_short_private_r19 import (  # noqa: E402
    ORACLE_SHA256,
    verified_oracle,
)
...
_LAUNCH_REL = "configs/r19_iceberg_short_paid_launch_v1.json"
_LAUNCH = ROOT / _LAUNCH_REL
_REGISTRATION_REL = "configs/r19_iceberg_short_registration_v1.json"
_BOUND_FILES = (
    "scripts/r19_iceberg_short_paid_runner.py",
    ...
    "src/rsicontext/analysis/iceberg_short_private_r19.py",
...
def _read_launch() -> tuple[dict[str, object], str]:
    ...
    if raw != _committed_bytes(_LAUNCH_REL):
        raise ValueError("R19 short launch differs from committed HEAD")
    value = json.loads(raw)
    required = {
        "schema_version", "scope", "registration_sha256", "bound_file_sha256",
    ...
    if (
        ...
        or value["scope"] != "r19-iceberg-short-paid-feasibility"
    ):
        raise ValueError("R19 short launch schema or scope differs")
    ...
    if (
        ...
        raise ValueError("R19 short registration differs from committed launch")
    bound = value["bound_file_sha256"]
    if not isinstance(bound, dict) or set(bound) != set(_BOUND_FILES):
        raise ValueError("R19 short bound producer set differs")
    for relative in _BOUND_FILES:
        current = (ROOT / relative).read_bytes()
        if current != _committed_bytes(relative) or bound[relative] != _sha(current):
            raise ValueError(f"R19 short producer differs from launch: {relative}")
    if (
        ...
        or value["task_call_cap"] != 2
        or value["task_local_plus_requested_ceiling"] != 4738
        ...
        or value["global_call_cap"] != 4
        or value["global_local_plus_requested_ceiling"]
        != 4738 + 2 * (canary.EXPECTED_INPUT_TOKENS + 2048)
        or value["auxiliary_call_cap"] != 0
    ):
        raise ValueError("R19 short launch identity or budget differs")
...
def _task_block(
    *,
    ...
    if not isinstance(requested, list) or len(requested) != len(short.CASE_ORDER):
        raise ValueError("R19 short registration lacks two requests")
    ...
    for case, item in zip(short.CASE_ORDER, requested, strict=True):
        if not isinstance(item, dict) or item.get("case") != case:
            raise ValueError("R19 short task request order differs")
        digest = item.get("prompt_sha256")
        if not isinstance(digest, str) or digest in registered:
            raise ValueError("R19 short prompt registration is invalid")
        registered[digest] = item
    request_text = build_iceberg_row_scan_sessions(
        source_root, case="authentic-pack"
    )[1].stages[1].documents[0].text
    if _sha(request_text.encode()) != registration["fixed_request_sha256"]:
        raise ValueError("R19 short S2 request changed before dispatch")
    active_case = ""

    def preflight(prompt: str) -> dict[str, object]:
        item = registered.get(_sha(prompt.encode()))
        if item is None or item["case"] != active_case:
            raise ValueError("R19 short worker prompt unregistered or out of order")
        return {
            "stage": active_case,
    ...
    for case in short.CASE_ORDER:
        active_case = case
        try:
            reply = worker(geometry_source.decision_user(request_text, short.RULES[case]))
        except Exception as exc:
            failure_type = type(exc).__name__
            break
        normalized = reply.strip()
        valid_format = normalized in set(oracle.values())
        correct = normalized == oracle[case]
        cases.append(
            {
                "case": case,
                "reply": reply,
        ...
        if not valid_format:
            ...
            break
        wrong_seen |= not correct
    usage = {
        key: journal.usage_summary()[key] - before[key]
        for key in ("input_tokens", "output_tokens", "unknown_usage_attempts")
    }
    completed = failure_type is None and not worker.failed and len(cases) == 2
    return {
        "schema_version": 1,
        "scope": "r19-iceberg-s2-only-feasibility",
        "status": "completed-pair" if completed else "stopped-on-task-failure",
...
def run_guarded(
    *,
    ...
    result: dict[str, object] = {
        "schema_version": 1,
        "scope": "r19-iceberg-short-paid-feasibility",
    ...
    try:
        ...
        if (
            ...
            raise ValueError("R19 short material or API request registration drifted")
        ...
        if pre.get("status") != "passed":
            ...
            if (
                ...
                raise RuntimeError("R19 short worker calls or usage differ from journal")
            ...
            if observed_usage["unknown_usage_attempts"] != 0:
            ...
            elif task["status"] != "completed-pair":
                ...
                if result["status"] in {
                    ...
                    if (
                        journal.task_attempts != 2
                        or journal.canary_attempts != 2
                        or journal.attempts != 4
                        or journal.planned_tokens != launch["global_local_plus_requested_ceiling"]
                    ):
                        raise RuntimeError("R19 short completed block lacks registered attempts")
                    _read_launch()
                    if registration != short.build_registration(
                        source_root, tokenizer_path, profile=profile, tokenizer=tokenizer
                    ):
                        raise RuntimeError("R19 short source or request changed during block")
